...
class Menu {
    ...
    create() {
        let cats = [];
        cats.push(pgame.add.sprite(0, 0, 'cat1'));
        cats.push(pgame.add.sprite(0, 0, 'cat2'));
        cats.push(pgame.add.sprite(0, 0, 'cat2'));
        cats.push(pgame.add.sprite(0, 0, 'cat2'));
        cats.push(pgame.add.sprite(0, 0, 'cat2'));
        cats.push(pgame.add.sprite(0, 0, 'cat2'));
        ...
        //initial position of cats on stage based on the selected cat
        function setToPosition(prime) {
            cats[prime].x = pgame.width / 2;

            //check if there is another cat available to display on the right side; if yes then position it
            if (prime<(totalCats-1)) {
                cats[prime + 1].x = pgame.width / 2 + 67 + 75;
                cats[prime + 1].scale.setTo(0.5,0.5);
            }

            //check if there is another cat available to display on the left side; if yes then position it
            if (prime > 0) {
                cats[prime - 1].x = pgame.width / 2 - 67 - 75;
                cats[prime - 1].scale.setTo(0.5,0.5);
            }
